...
class AutoPick(Toplevel):
    def __init__(self, master=None):
        ...
        not_select_bar = ttk.Scrollbar(self, orient="vertical", command=self.not_selected.yview)
        ...
        selected_bar = ttk.Scrollbar(self, orient="vertical", command=self.not_selected.yview)
        ...
        ttk.Button(button_frame, text="添加英雄>>", command=self.add_champion).grid(row=1)
        ttk.Button(button_frame, text="<<删除英雄", command=self.remove_champion).grid(row=2)
        ttk.Button(button_frame, text="向上移动↑↑", command=self.move_up).grid(row=3)
        ttk.Button(button_frame, text="向下移动↓↓", command=self.move_down).grid(row=4)
        ...
        if not CONF.AUTO_PICK_CACHE.exists():
            ...
            with CONF.AUTO_PICK_CACHE.open("w", encoding="utf8") as f:
                json.dump(self.champions, f, ensure_ascii=False)
        else:
            # 显示英雄列表
            with CONF.AUTO_PICK_CACHE.open("r", encoding="utf8") as f:
        ...
        for champion_id, champion_name in self.champions["not-selected"].items():
            self.not_selected.insert("", "end", text=champion_name, values=champion_id)
...
class UI(Tk):
    def __init__(self, task: Callable):
        ...
        ttk.Checkbutton(
            self, text="自动确认", command=set_auto_confirm, variable=auto_pick_var
        ).grid(row=0, column=0, sticky="we")

        # 自动选择英雄开关
        auto_pick_var = BooleanVar(value=CONF.AUTO_PICK_SWITCH)
        ttk.Checkbutton(
            self, text="自动选英雄", command=set_auto_pick, variable=auto_pick_var
        ).grid(row=0, column=1, sticky="we")

        # 自动分析开关
        auto_analysis_var = BooleanVar(value=CONF.AUTO_ANALYSIS)
        ttk.Checkbutton(
            self, text="战绩分析", command=set_auto_analysis, variable=auto_analysis_var
        ).grid(row=1, column=0, sticky="we")

        # 保存队友最近20场比赛记录
        save_match_var = BooleanVar(value=CONF.SAVE_MATCH)
        ttk.Checkbutton(
            self, text="保存队友记录", command=set_save_match, variable=save_match_var
        ...
        # 启动按钮
        ttk.Button(self, text="启动助手", command=self.start).grid(row=1, column=2, sticky="we")
        ...
        # 滚动条
        vertical_bar = ttk.Scrollbar(self, orient="vertical", command=text.yview)
        vertical_bar.grid(row=2, column=3, sticky="ns")

        text.configure(yscrollcommand=vertical_bar.set)
        logger.add(
            lambda msg: text.insert("end", msg) or text.see("end"),
